...
# HOST records per page (per the docs — not detections; a host can carry many
# detections, so actual page size varies with estate density). Lower = faster
# pages, less likely to time out, more round trips. Reduce this before raising
# the read timeout — it attacks the cause rather than the symptom.
...
# ---- tag scope: referenced by NAME (TAG_SET_BY=name, the default).
# Names are matched LITERALLY — case and spacing exactly as the API returns
# them via show_tags=1 (hence "DESKTOP", not "Desktop"). Earlier 1904 errors
# turned out to be tag SCOPE permissions, not name matching, so names are kept
# for readability. TAG_SET_BY=id remains available and is the sturdier choice
# if tags are ever renamed under a running job.
TAG_SET_BY = env("TAG_SET_BY", "name")


def _tag_list(raw):
    """Normalise a comma-separated tag list for the API's {tag1,tag2} format:
    drop empty entries and strip whitespace AROUND each entry, preserving
    whitespace INSIDE a name like 'Env: PROD'. 'a, b ,,c' -> 'a,b,c'."""
    return ",".join(s for s in (part.strip() for part in (raw or "").split(",")) if s)


TAG_INCLUDE = _tag_list(env("TAG_INCLUDE", "Env: PROD"))
TAG_EXCLUDE = _tag_list(env("TAG_EXCLUDE", "FPS-bootmgmt,Bastions,bootmgmt,DESKTOP"))
...
DESKTOP_TAG = _tag_list(env("DESKTOP_TAG", "DESKTOP"))

# Assets carrying these tags are FLAGGED in the ESTATE summary — one column
# per tag — and marked in the per-vulnerability issues. Purely informational:
# never changes what is pulled, ticketed or closed. Deliberately absent from
# the desktop summary, which is already scoped to untracked assets.
# Matching is case-insensitive here (unlike the API filters, where Qualys
# matches literally): a mis-cased flag is a cosmetic miss, not lost data.
FLAG_TAGS = [s.strip() for s in
             env("FLAG_TAGS", "FPS-DC-PROD,EKS-FPS_Prod").split(",") if s.strip()]
_FLAG_LOOKUP = {tag.lower(): tag for tag in FLAG_TAGS}   # lower -> display
FLAG_LABEL = env("FLAG_LABEL", "FLAGGED")
DESKTOP_SUMMARY = env("DESKTOP_SUMMARY", "true").lower() == "true"


def _validate_tag_config():
    """Fail at startup, not mid-pull. A tag NAME left behind in an ID-mode
    list would otherwise surface as Qualys error 1904 with no hint why."""
    if TAG_SET_BY not in ("name", "id"):
        log.error("TAG_SET_BY must be 'name' or 'id', got %r", TAG_SET_BY)
        sys.exit(2)
    if TAG_SET_BY == "id":
        bad = [
            entry
            for lst in (TAG_INCLUDE, TAG_EXCLUDE, DESKTOP_TAG)
            for entry in lst.split(",")
            if entry and not entry.isdigit()
        ]
        if bad:
            log.error(
                "TAG_SET_BY=id but these entries are not numeric tag IDs: %s. "
                "A tag NAME probably survived the switch. Get IDs from "
                "POST /qps/rest/2.0/search/am/tag or the tag's URL in the UI.",
                ", ".join(repr(b) for b in bad),
            )
            sys.exit(2)
    for label, lst in (("TAG_INCLUDE", TAG_INCLUDE), ("TAG_EXCLUDE", TAG_EXCLUDE),
                       ("DESKTOP_TAG", DESKTOP_TAG)):
        if "%" in lst:
            log.error(
                "%s contains a %% escape (%r). Values must be plain text with "
                "literal commas — never pre-encode (%%2C etc.); encoding "
                "happens exactly once when the request is built.", label, lst,
            )
            sys.exit(2)
    if DESKTOP_SUMMARY and not DESKTOP_TAG:
        log.error(
            "DESKTOP_SUMMARY is enabled but DESKTOP_TAG is empty — set the "
            "desktop tag ID, or disable the desktop summary."
        )
        sys.exit(2)


_validate_tag_config()
...
SUMMARY_DATE_RE = re.compile(r"qualys-qds-summary-date:\s*(\d{4}-\d{2}-\d{2})")
# QIDs first seen on the summary's date. Persisted so that re-running the job
# later the same day (which updates the summary in place) does not erase the
# morning's "new today" set — by then those issues exist and would otherwise
# no longer look new.
SUMMARY_NEW_RE = re.compile(r"qualys-qds-new:\s*([^\s>]*)")
...
def _detection_params(tag_include, tag_exclude):
    ...
    if tag_include or tag_exclude:
        params["use_tags"] = "1"
        params["tag_set_by"] = TAG_SET_BY
        if tag_include:
            params["tag_include_selector"] = "any"
            params["tag_set_include"] = _tag_list(tag_include)
        if tag_exclude:
            params["tag_exclude_selector"] = "any"
            params["tag_set_exclude"] = _tag_list(tag_exclude)
    return params


def _encode_query(params):
    """Encode the query string OURSELVES, in exactly one pass, keeping commas
    literal. The docs' {tag1,tag2} samples use bare commas, and letting the
    HTTP library encode them to %2C has been seen to break list parsing.
    Handing requests a pre-built string stops it re-encoding anything.

    Corollary for config: values must be PLAIN text — never put %2C (or any
    %-escape) in TAG_EXCLUDE/.env/workflow env. Pre-encoded values get encoded
    again here (%2C -> %252C) and the call fails confusingly.
    """
...
def qualys_pull(tag_include=None, tag_exclude=None):
    ...
    params = _encode_query(_detection_params(tag_include, tag_exclude))
    log.info(
    ...
    while url:
        ...
        for chunk in resp.iter_content(chunk_size=65536):
            ...
            if first_byte is None:
                ...
                log.info(
                    "  first byte after %.1fs (NB: Qualys sends <!-- keep-alive"
                    " --> filler every 30-40s during generation, so early bytes"
                    " may not be real data)",
                    first_byte,
                )
            chunks.append(chunk)
            received += len(chunk)
...
def _raise_on_qualys_error(root):
    ...
    if root.tag == "SIMPLE_RETURN" or root.find("./RESPONSE/CODE") is not None:
        ...
        if code == "1904":
            hint = (
                "\nThe tag filter matched no scannable assets in this account's "
                "view. For a non-Manager API account the usual cause is TAG "
                "SCOPE, not the tag reference itself — tags are scoped objects, "
                "so being able to SEE a tag on a host (show_tags=1) does not "
                "grant the right to SELECT by it. Check, in order:\n"
                "  1. Is the tag assigned to this user's scope? As a Manager: "
                "Administration -> find the user -> Quick Actions -> 'Add Tags "
                "To Scopes', or Roles and Scopes -> Edit Scope. The user must "
                "RE-LOGIN afterwards (scope is cached per session).\n"
                "  2. Does the user have the 'View All Tags' permission "
                "(Administration utility)? Without it a sub-user cannot use "
                "tags created by other users.\n"
                "  3. Tag scope INTERSECTS asset availability: the tag's assets "
                "must also be reachable via the user's asset groups / business "
                "unit. Both are required, not either.\n"
                "  4. If TAG_SET_BY=name, the name must match EXACTLY (case and "
                "spacing). Numeric IDs avoid that class of error entirely — but "
                "note IDs do NOT bypass scope, so 1904 by ID points at 1-3.\n"
                "Differential test: try a tag you have OBSERVED on a visible "
                "host. If that also 1904s, it is scope/permissions; if it works, "
                "the original tag simply has no in-scope assets."
            )
        raise RuntimeError(f"Qualys API error {code}: {text}{hint}")
    ...
    for host in root.findall("./RESPONSE/HOST_LIST/HOST"):
        ...
        for det in host.findall("./DETECTION_LIST/DETECTION"):
            ...
            yield {
                "asset_key": asset_key,
                "tags": tags,
                "flag_matches": sorted({_FLAG_LOOKUP[tg.lower()] for tg in tags
                                        if tg.lower() in _FLAG_LOOKUP}),
                "endpoint": endpoint,
                "host_id": host_id,
    ...
    for d in detection_iter:
        ...
        if prev is None or d["qds"] > prev["qds"]:
            g["assets"][d["asset_key"]] = {
                ...
                "flag_matches": sorted(set(d.get("flag_matches", []))
                                       | set((prev or {}).get("flag_matches", []))),
            ...
            # Lower-scoring instance: still contributes its endpoints, its flag
            # status, and can carry the earliest sighting.
            prev["endpoints"] = endpoints
            prev["flag_matches"] = sorted(set(prev.get("flag_matches", []))
                                          | set(d.get("flag_matches", [])))
...
def flagged_assets(group, tag=None):
    """Asset keys carrying a specific flag tag, or any flag tag when tag=None."""
    def _matches(asset):
        matched = asset.get("flag_matches") or []
        return (tag in matched) if tag else bool(matched)

    return sorted(k for k, a in group["assets"].items() if _matches(a))
...
def assets_fingerprint(group):
    ...
    parts = sorted(
        ...
        f"{'!' + ','.join(a.get('flag_matches', [])) if a.get('flag_matches') else ''}"
...
def build_issue(group):
    ...
    table = _pre_table(
        ("SERVICE/ASSET", "INSTANCE", "IP", "ENDPOINTS", FLAG_LABEL.upper(),
         "QDS", "FIRST FOUND", "LAST FOUND"),
        [
            (
                a["asset_key"],
                a["host_label"],
...
def build_summary(groups, existing, today, scope="estate", prior_new=None):
    ...
    # NEW = no tracked issue for this QID when the run started. Union with any
    # set already recorded on today's summary, so a same-day re-run keeps
    # showing what arrived today. Desktop has no tracked issues, so the
    # distinction is not available there and the section is omitted.
    new_keys = set() if desktop else (
        {k for k in groups if k not in existing} | set(prior_new or ())
    ) & set(groups)
    ordered = sorted(
        groups.values(),
    ...
    flag_counts = {tag: sum(1 for g in ordered if flagged_assets(g, tag))
                   for tag in FLAG_TAGS}
    new_crit = sum(1 for k in new_keys
                   if groups[k]["qds_severity"] == "CRITICAL")
    ...
    for g in ordered:
        ...
        # One column per flag tag, showing how many of this QID's assets carry
        # it. Omitted entirely from the desktop summary.
        flag_cells = () if desktop else tuple(
            str(len(flagged_assets(g, tag))) or "-" if flagged_assets(g, tag) else "-"
            for tag in FLAG_TAGS
        )
        rows.append((
            *(() if desktop else ("** NEW **" if g["key"] in new_keys else "",)),
            g["qid"],
            g["qds_severity"],
            g["max_qds"],
            len(g["assets"]),
            ...
            *flag_cells,
            ...
            "not tracked" if desktop else (
                f"#{meta['number']}" if meta else "pending"),
        ))
    headers = (*(() if desktop else ("",)), "QID", "BAND", "MAX QDS", "ASSETS", "ENDPOINTS",
               *(() if desktop else tuple(tag.upper() for tag in FLAG_TAGS)),
               "FIRST FOUND", "ISSUE")
    table = _pre_table(headers, rows) if rows else (
        "(nothing currently tracked at/above threshold)")

    # A section of its own, above everything else — the point of the summary
    # is that today's arrivals should not have to be hunted for.
    new_block = ""
    if new_keys:
        new_ordered = sorted(
            (groups[k] for k in new_keys),
            key=lambda g: (g["qds_severity"] != "CRITICAL", -g["max_qds"]),
        )
        new_table = _pre_table(
            ("QID", "BAND", "MAX QDS", "ASSETS",
             *(tag.upper() for tag in FLAG_TAGS), "FIRST FOUND"),
            [
                (
                    g["qid"], g["qds_severity"], g["max_qds"], len(g["assets"]),
                    *(str(len(flagged_assets(g, tag))) if flagged_assets(g, tag)
                      else "-" for tag in FLAG_TAGS),
                    min(filter(None, (a["first_found"] for a in g["assets"].values())),
                        default="n/a"),
                )
                for g in new_ordered
            ],
        )
        new_block = (
            f"\n## NEW THIS RUN — {len(new_keys)} "
            f"vulnerabilit{'y' if len(new_keys) == 1 else 'ies'} not previously "
            f"tracked\n\n```\n{new_table}\n```\n"
        )
    ...
    if desktop:
        ...
        headline = (
            ...
            f"across {nassets} assets"
            + ("\n" + "  |  ".join(
                f"**{tag}:** {n} vulnerabilit{'y' if n == 1 else 'ies'}"
                for tag, n in flag_counts.items() if n)
               if any(flag_counts.values()) else "")
        )
        scope_note = ""
    banner = ""
    if not desktop:
        banner = (
            f"# {len(new_keys)} NEW vulnerabilit"
            f"{'y' if len(new_keys) == 1 else 'ies'} today"
            + (f" — {new_crit} CRITICAL" if new_keys and new_crit else "")
            + "\n\n"
        ) if new_keys else "**No new vulnerabilities today.**\n\n"

    body = f"""{banner}**Date:** {today}
{headline}
{scope_note}

{new_block}### All tracked vulnerabilities
```
{table}
```
{absent_block}
...
<!-- qualys-qds-summary-date: {today} -->
<!-- qualys-qds-new: {','.join(sorted(new_keys)) or '-'} -->
"""
    return title, body


def publish_summary(groups, existing, today, scope="estate"):
    """Ensure exactly ONE open summary per scope, for today.

    * A summary already open for TODAY is updated in place — re-running the job
      (manual trigger, double-fired schedule, repeated testing) must not spawn
      duplicates or churn create/close pairs.
    * Summaries open for an EARLIER day are closed as superseded. That
      supersession is the only closure this job performs.

    Publish-then-close ordering: a failure between the two steps leaves an
    extra open summary (self-correcting next run) rather than none.
    """
    previous = find_open_summaries(scope)

    todays, older = [], []
    for number, prev_body in previous:
        match = SUMMARY_DATE_RE.search(prev_body or "")
        (todays if match and match.group(1) == today else older).append(
            (number, prev_body)
        )

    # Carry forward what today's summary already recorded as new, so a second
    # run today does not report "no new vulnerabilities" simply because the
    # issues it created this morning now exist.
    prior_new = set()
    if todays:
        marker = SUMMARY_NEW_RE.search(todays[0][1] or "")
        if marker and marker.group(1) != "-":
            prior_new = {k for k in marker.group(1).split(",") if k}

    title, body = build_summary(groups, existing, today, scope, prior_new)

    if DRY_RUN:
        action = ("update #%d in place" % todays[0][0]) if todays else "publish a new summary"
        log.info("[dry-run] would %s for scope %r and close %d superseded",
                 action, scope, len(older))
        return
    ...
    if todays:
        ...
        log.info("Updated today's %s summary #%s in place", scope, number)
        current = number
        # Any extra same-day summaries are duplicates; supersede them too.
        older.extend(todays[1:])
    else:
        resp = _github_request(
            "POST",
            f"/repos/{GITHUB_REPO}/issues",
        ...
        resp.raise_for_status()
        current = resp.json()["number"]
        log.info("Published %s daily summary issue #%s", scope, current)

    for number, prev_body in older:
        _close_summary_issue(number, prev_body, current)
