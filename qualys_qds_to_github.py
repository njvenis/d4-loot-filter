...
# ---- tag scope: referenced by numeric tag ID (TAG_SET_BY=id, the default).
# IDs cannot be miscased or misspaced and survive renames — the literal-name
# mismatches ("Desktop" vs "DESKTOP") that produced Qualys error 1904 cannot
# recur. TAG_SET_BY=name still works; names are then matched LITERALLY,
# exactly as the API returns them (show_tags=1). Keep the human-readable tag
# names in a comment wherever the IDs are configured.
TAG_SET_BY = env("TAG_SET_BY", "id")
...
TAG_INCLUDE = _tag_list(env("TAG_INCLUDE", required=True))   # Env: PROD's tag ID
TAG_EXCLUDE = _tag_list(env("TAG_EXCLUDE", ""))              # untracked-asset tag IDs
...
DESKTOP_TAG = _tag_list(env("DESKTOP_TAG", ""))
