...
def _raise_on_qualys_error(root):
    ...
    if root.tag == "SIMPLE_RETURN" or root.find("./RESPONSE/CODE") is not None:
        ...
        if code == "1904":
            hint = (
                "\nThe tag include filter matched no scannable assets in this "
                "account's view. Check, in order:\n"
                "  1. Tag name is EXACT — case, spacing, punctuation ('Env: PROD' "
                "vs 'Env:PROD' vs 'env: prod'). Copy it verbatim from the Tags "
                "view in the Qualys UI.\n"
                "  2. Assets carrying the tag are within THIS API account's "
                "scope — a Reader sees only its assigned asset groups, so "
                "correctly-tagged assets outside them still yield 1904.\n"
                "  3. Assets are tagged with this literal tag, not only a "
                "child/parent of it.\n"
                "  4. Bisect: retry with ONLY tag_set_include, then only "
                "tag_set_exclude, to isolate the failing term.\n"
                "tag_set_by=id with the tag's numeric ID sidesteps name-matching "
                "entirely and survives tag renames."
            )
