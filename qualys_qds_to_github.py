...
SUMMARY_MARKER = "<!-- qualys-qds-summary -->"
...
def qualys_pull():
    ...
    while url:
        ...
        detections = list(_parse_detections(root))
        hosts = len(root.findall("./RESPONSE/HOST_LIST/HOST"))
        log.info(
        ...
        if hosts and not detections:
            log.warning(
                "Page %d returned hosts but no QDS detections — check that the "
                "subscription returns QDS (show_qds) and that QDS_MIN=%d is not "
...
def find_open_summaries():
    """All open issues carrying BOTH the summary label and the summary marker."""
    resp = _github_request(
        "GET",
        f"/repos/{GITHUB_REPO}/issues",
...
def _close_summary_issue(number, body, superseded_by):
    ...
    """
    if SUMMARY_MARKER not in (body or ""):
        log.error(
...
def build_summary(groups, existing, today):
    """The day's report: a preformatted table of everything currently tracked
    at/above threshold (QID as the primary key, assets grouped beneath it in
    the per-vuln issues), plus a section for tracked-but-absent QIDs awaiting
    ...
    body = f"""**Date:** {today}
**Tracked at/above threshold (QDS >= {QDS_MIN}):** {len(ordered)} vulnerabilities ({ncrit} critical, {nhigh} high) across {nassets} assets
...
{absent_block}
---
_Published daily; yesterday's summary is closed as superseded — the only closure this job ever performs. Per-vulnerability issues are linked in the ISSUE column and are never closed automatically: finding closure is a human decision._
{SUMMARY_MARKER}
"""
    return title, body


def publish_summary(groups, existing, today):
    """Create today's summary issue, then close any previous open summaries as
    superseded. Publish-then-close, so a failure between the two steps leaves
    an extra open summary (self-corrects next run) rather than no summary."""
    title, body = build_summary(groups, existing, today)
    previous = find_open_summaries()
    if DRY_RUN:
        log.info(
    ...
    resp = _github_request(
        "POST",
        f"/repos/{GITHUB_REPO}/issues",
        json={"title": title, "body": body, "labels": [SUMMARY_LABEL]},
    )
    resp.raise_for_status()
    new_number = resp.json()["number"]
    log.info("Published daily summary issue #%s", new_number)
...
def main():
    ...
    if not groups and existing:
        ...
        publish_summary(groups, existing, today.isoformat())
