...
def main():
    ...
    if args.sample:
        ...
        if dets == 0:
            ...
            print("       The API account likely cannot see any assets. Check its")
            print("       asset-group scope and role in Qualys (Users -> User).")
        elif not scores:
            print(f"{BAD} Detections returned, but NOT ONE carried a <QDS> element.")
            print("       This is an entitlement/visibility problem, not a threshold")
            print("       one. TruRisk/QDS is not being returned for this")
            print("       subscription or this API account. The job cannot work")
            print("       until Qualys Support enables it — raise a case.")
        else:
            bands = {
